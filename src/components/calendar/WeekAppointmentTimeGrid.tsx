      ...
      <table className="w-full min-w-[44rem] table-fixed border-collapse">
        <thead>
          <tr className="border-b border-stone-200 bg-stone-50/90 dark:border-stone-800 dark:bg-stone-800/50">
            <th
              scope="col"
              className="sticky left-0 z-20 w-16 border-r border-stone-200 bg-stone-50/95 px-2 py-3 sm:w-20"
            />
            {columns.map((column) => (
        ...
        <tbody>
          {timeSlots.map((slotMinutes) => {
            ...
            return (
            <tr key={slotMinutes} className="border-b border-stone-100 last:border-b-0">
              <th
                scope="row"
                className="sticky left-0 z-10 border-r border-stone-200 bg-white px-2 py-0 text-right align-top sm:px-3"
              >
                <span className="block py-2 text-[10px] font-medium tabular-nums text-stone-400 sm:text-xs">
              ...
              {columns.map((column) => {
                ...
                return (
                  <td
                    ...
                    className={cn(
                      'border-r border-stone-100 align-top p-0.5 last:border-r-0 sm:p-1',
                      column.isToday && 'bg-brand-50/25',
                    )}
