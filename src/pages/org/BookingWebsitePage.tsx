...
import { getBookingPageUrl } from '@/lib/public-booking';
...
export function BookingWebsitePage() {
  ...
  if (!publicBookingEnabled) {
    return (
      <div className="mx-auto max-w-3xl">
        ...
        <Card>
          <CardContent className="space-y-3 pt-6 text-sm text-stone-600">
  ...
  const liveUrl =
    hostingMode === 'subdomain' && website.deployedSiteUrl
  ...
  return (
    <div className="mx-auto min-w-0 max-w-3xl space-y-6">
            ...
            <div className="min-w-0 space-y-4">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 flex min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch">
                  <code className="block min-w-0 flex-1 break-all rounded-md border border-stone-200 bg-white px-3 py-2 text-xs text-brand-700 sm:text-sm">
                ...
                <p className="mt-2 break-words text-xs text-stone-500">
                  Format: <span className="font-mono">yoursite.com/book/{data.organizationSlug}</span>
                </p>
              </div>
            </div>
            <BookingPagePreview
            ...
            {data.siteTemplates.map((template) => (
              <div key={template.id} className="min-w-0 space-y-3">
                <button
                  ...
                  className={cn(
                    'w-full rounded-lg border bg-white p-4 text-left transition-colors hover:border-brand-300',
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-stone-600">
            ...
            <span className="font-mono text-brand-700">{data.organizationSlug}.viselle.app</span> with
            optional custom design. You can add this anytime — your included booking link keeps working until you switch.
          </p>
          ...
          {data.subdomainHostingEnabled ? (
            <div className="space-y-3 rounded-lg border border-stone-200 bg-stone-50 p-4">
              ...
              <p className="text-sm font-medium text-brand-700">{subdomainPreview}</p>
              {hostingMode !== 'subdomain' ? (
                ...
                <Button asChild variant="outline">
                  <a href={subdomainPreview} target="_blank" rel="noreferrer">
                    <ExternalLink className="h-4 w-4" />
                    Open subdomain site
                  </a>
                </Button>
      ...
      <p className="text-xs text-stone-500">
        ...
        <a href="/#websites" className="text-brand-700 hover:underline">
          booking page options on our site
        </a>{' '}
        or{' '}
        <Link to={contactPath({ interest: 'api' })} className="text-brand-700 hover:underline">
